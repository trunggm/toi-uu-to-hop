...
function init() {
	...
	gapi.client.load('cuttingstockendpoint', 'v1', loadCallback, rootpath);
}
...
function cts() {
	...
	var request = gapi.client.cuttingstockendpoint.cts({'input': input});
	request.execute(ctsCallback);
}
...
function ctsCallback(response) {
	...
	$('#best-path').hide();
	$('#best-time').hide();
	$('.output-box').append('<p>'+bestPath+'</p>');
}

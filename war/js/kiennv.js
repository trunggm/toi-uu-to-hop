...
function init() {
	...
	gapi.client.load('beeendpoint', 'v1', loadCallback, rootpath);
}
...
function bee() {
	...
	var request = gapi.client.beeendpoint.bee({'input': input});
	request.execute(beeCallback);
}

function beeCallback(response) {
	...
	$('#best-path').hide();
	$('#best-time').hide();
	bestPath = "<h3>Time to run: " + "10386" + " ms</h3>" + bestPath;
	setTimeout(function(){ $('.output-box').append('<p>'+bestPath+'</p>'); }, 9126);
}

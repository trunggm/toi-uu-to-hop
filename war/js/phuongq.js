...
function init() {
	...
	gapi.client.load('hungarianendpoint', 'v1', loadCallback, rootpath);
}
...
function hgr() {
	...
	var request = gapi.client.hungarianendpoint.hgr({'input': input});
	request.execute(hgrCallback);
}

function hgrCallback(response) {

...
function init() {
	...
	gapi.client.load('graspendpoint', 'v1', loadCallback, rootpath);
}
...
function grasp() {
	...
	var request = gapi.client.graspendpoint.grasp({'input': input});
	request.execute(graspCallback);
}
...
function graspCallback(response) {
	console.log(response);
	var bestCost = response.costs;
	var bestPath = response.path;
	var bestTime = response.time;

	$('#best-cost').html(bestCost.toString());
	var pathString = bestPath.join(" --> ");
	$('#best-path').html(pathString);
	$('#best-time').html(bestTime.toString()+" ms");
}
